  ...
  const addSpeakingTask = () => {
    if (speakingTasks.length < maxParts) {
      setSpeakingTasks([
        ...speakingTasks,
        {
          part: `S_TASK${speakingTasks.length + 1}`,
          prompt: "",
        },
      ])
    }
        ...
        <div className="accordion">
          {speakingTasks.map((task, taskIndex) => (
            <div key={taskIndex} className="accordion-item">
              <div className="accordion-header" onClick={() => toggleAccordion(`speaking-task-${taskIndex}`)}>
                <span>
              ...
              <div id={`speaking-task-${taskIndex}`} className="accordion-content">
                ...
                <div className="form-actions">
                  <button type="button" className="btn btn-sm btn-danger" onClick={() => removeSpeakingTask(taskIndex)}>
                    Remove Task
                  </button>
                </div>
              </div>
